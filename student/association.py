...
class Association:
    ...
    def associate(self, track_list, meas_list, KF):
        ...
        meas_len = len(meas_list)
        track_len = len(track_list)

        if meas_len > 0:
            self.unassigned_meas = list(range(meas_len))
        if track_len > 0:
            self.unassigned_tracks = list(range(track_len))
        if meas_len > 0 and track_len > 0: 
            self.association_matrix = np.inf * np.ones((track_len, meas_len))
        
        for i in range(track_len):
            track = track_list[i]
            for j in range(meas_len):
                meas = meas_list[j]
