...
class Sensor:
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            # project camera cord to image cord
            hx = np.zeros((2,1))
            if pos_sens[0] == 0:
                raise NameError('Divided number cannot be zero')
            else:
...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            camera_i = params.sigma_cam_i
            camera_j = params.sigma_cam_j
            self.z = np.zeros((sensor.dim_meas,1)) # measurement vector
            self.z[0] = z[0]
            self.z[1] = z[1]
            self.R = np.matrix([[camera_i**2, 0], # measurement noise covariance matrix
                                [0, camera_j**2]])
            self.width = 0
            self.length = z[3]
            self.height = z[2]
